...
def run_sweep(config_file="config.yaml"):
    ...
    def run_one_condition(sim_params: Dict[str, Any], run_id: int) -> Dict[str, Any]:
        ...
        init_time = time.time()
        print(f"  Initialization took {init_time - start:.1f}s")
        
        max_steps = sim_params.get("max_steps", None)
        kmc_sim.run_gillespie(max_time=max_time_s, max_steps=max_steps)
        end_time = time.time()
        print(f"  Simulation took {end_time - init_time:.1f}s")
        print(f"  Total: {end_time - start:.1f}s")
